...
impl EncoderPrefixes {
    ...
    pub fn br_hint_not_taken(&self) -> bool {
        unsafe { self.inner.s.br_hint_not_taken() != 0 }
    }
    pub fn br_hint_taken(&self) -> bool {
        unsafe { self.inner.s.br_hint_taken() != 0 }
    }
    pub fn rep(&self) -> bool {
        unsafe { self.inner.s.rep() != 0 }
    }
    pub fn repne(&self) -> bool {
        unsafe { self.inner.s.repne() != 0 }
    }
    ...
    pub fn set_br_hint_not_taken(&mut self, v: bool) {
        unsafe { self.inner.s.set_br_hint_not_taken(if v { 1 } else { 0 }) }
    }
