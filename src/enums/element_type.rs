...
impl From<xed_operand_element_type_enum_t> for ElementType {
    fn from(x: xed_operand_element_type_enum_t) -> Self {
        Self::from_u32(x).unwrap_or(ElementType::Invalid)
    }
}

impl From<ElementType> for xed_operand_element_type_enum_t {
    fn from(x: ElementType) -> Self {
        x as Self
    }
}
