...
impl From<xed_flag_action_enum_t> for FlagAction {
    fn from(x: xed_flag_action_enum_t) -> Self {
        Self::from_u32(x).unwrap_or(FlagAction::Invalid)
    }
}

impl From<FlagAction> for xed_flag_action_enum_t {
    fn from(x: FlagAction) -> Self {
        x as Self
    }
}
